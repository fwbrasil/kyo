...
object IOStream:
    def toJavaPublisher[E, A: Flat, Ctx](stream: Stream[A, Abort[E] & Async & Ctx])(
        ...
        IO.Unsafe {
            // val publisher = new Flow.Publisher[A]:
            //     def subscribe(subscriber: Subscriber[? >: A]): Unit =
            //         val requests = Channel.Unsafe.init[Long](Int.MaxValue)
            //         val subscription = new Flow.Subscription:
            //             def request(n: Long): Unit = discard(requests.offer(n))
            //             def cancel(): Unit         = discard(requests.close())

            //         val fiber =
            //             Async.run {
            //                 stream.runForeach { v =>
            //                     requests.safe.take.map { n =>
